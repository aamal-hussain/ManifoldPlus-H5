	...
	std::vector<Vector3i> &triangles)
{
	...
	triangles.clear();
	FT half_len = (nvertices[nface_indices[0][1]] - nvertices[nface_indices[0][0]]).norm() * 0.5;
	for (int i = 0; i < (int)nface_indices.size(); ++i)
	{
		int t = 0;
			...
			for (int dimy = -1; dimy < 2; dimy += 2)
			{
				for (int dimz = -1; dimz < 2; dimz += 2)
				{
					...
					if (tree_->IsExterior(p1))
					{
						...
						int id2 = vcolor[ind2];
						int id3 = vcolor[ind3];
						Vector3 norm = (nvertices[id2] - nvertices[id1]).cross(nvertices[id3] - nvertices[id1]);
		 ...
		 it != evens.end(); ++it)
	{
		...
		for (auto it1 = p.begin();
			 it1 != p.end(); ++it1)
		{
			if (dir.dot(nvertices[triangles[it1->first][(it1->second + 1) % 3]] - nvertices[i]) < 0)
			{
