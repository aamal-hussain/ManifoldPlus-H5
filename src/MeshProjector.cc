	...
	while (true)
	{
		...
		*/
		...
		for (int i = num_active_ - 1; i >= 0; --i)
		{
			...
			if (std::abs(d1 - d0) > ZERO_THRES || vn.dot(out_N_.row(vid)) < 1 - ZERO_THRES)
			{
		...
		else
		{
			...
			for (int i = 0; i < num_active_temp; ++i)
			{
				...
				{
					sqrD_[vid] = (target_V_.row(vid) - out_V_.row(vid)).squaredNorm();
				}
			}
		}
	...
	for (int i = 0; i < A.size(); ++i)
	{
		...
		for (int j = 0; j < A.size(); ++j)
		{
			...
			if (B[j] < ZERO_THRES && denominator >= ZERO_THRES)
			{
				...
				if (constraint_size == 1 && constraints[0].cross(A[j]).norm() < ZERO_THRES)
